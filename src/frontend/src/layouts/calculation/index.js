...
function Calculation() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const normalizedLocations = Array.isArray(locations) 
        ...
        : [];
      DataTrazas.crear({ accion: "Vió los Cálculos" });
  ...
  const applyFilters = () => {
    if (!filtros.fechaInicio && !filtros.fechaFin && !filtros.ubicacion) {
      DataTrazas.crear({ accion: "No filtró correctamente en Cálculos" });
    ...
    if (filtros.fechaInicio && filtros.fechaFin && dayjs(filtros.fechaInicio).isAfter(filtros.fechaFin)) {
      DataTrazas.crear({ accion: "Puso la fecha al Filtrar Cálculos" });
  ...
  const clearFilters = () => {
    DataTrazas.crear({ accion: "Limpió los filtos de  Cálculos" });
  ...
  const handleExportPDF = () => {
    if (calculos.length === 0) {
      DataTrazas.crear({ accion: "No hay datos para exportar en Cálculos" });
      mostrarMensaje("No hay datos para exportar", 'warning');
      return;
    }
    DataTrazas.crear({ accion: "Ha exportado Pdf los Cálculos" });
    PDFExporter.exportCalculations(calculos, filtros);
  };

  const handlePrint = () => {
    if (calculos.length === 0) {
      DataTrazas.crear({ accion: "Ha impreso los Cálculos" });
